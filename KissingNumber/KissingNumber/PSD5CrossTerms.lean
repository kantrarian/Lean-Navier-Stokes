...
-- ============================================================
-- Helper lemmas (shared with PSD4CrossTerms)
-- ============================================================
...
private lemma factor4 (f g h k : Fin 8 → ℝ) :
    ∑ a : Fin 8, ∑ b : Fin 8, ∑ c : Fin 8, ∑ d : Fin 8,
      f a * g b * h c * k d =
...
-- ============================================================
-- Definitions: A5, B5, C5 for degree-5 trace-free tensors
-- ============================================================

/-- Raw 5th moment tensor: x_a * x_b * x_c * x_d * x_e -/
private def A5 (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.1 * x p.2.1 * x p.2.2.1 * x p.2.2.2.1 * x p.2.2.2.2

/-- 1-delta correction: C(5,2) = 10 terms, each = (3 x-coords) × δ_{pair} -/
private def B5 (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
...
private def C5 (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  -- unpaired a (p.1)
...
private def phi5 (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  A5 x p - (1 : ℝ) / 14 * B5 x p + (1 : ℝ) / 168 * C5 x p

private lemma phi5_product (x y : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) :
    phi5 x p * phi5 y p =
    A5 x p * A5 y p
    - (1/14) * (A5 x p * B5 y p + B5 x p * A5 y p)
    + (1/168) * (A5 x p * C5 y p + C5 x p * A5 y p)
    + (1/196) * (B5 x p * B5 y p)
    - (1/2352) * (B5 x p * C5 y p + C5 x p * B5 y p)
    + (1/28224) * (C5 x p * C5 y p) := by
  simp only [phi5]; ring

-- ============================================================
-- Cross-term sums (with sorry placeholders to be filled in)
-- ============================================================

private lemma sum_AA (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, A5 x p * A5 y p =
  ...
  rw [show ∑ a : Fin 8, x.ofLp a * y.ofLp a * s * s * s * s =
...
private lemma sum_AB (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, A5 x p * B5 y p =
  ...
  -- 4 canonical AC-normal forms: doubled at x_1 (4×), x_2 (3×), x_3 (2×), x_4 (1×)
  have kA : ∀ a b c d : Fin 8,
...
private lemma sum_BA (x y : EuclideanSpace ℝ (Fin 8)) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5 x p * A5 y p =
    10 * (@inner ℝ _ _ x y) ^ 3 := by
...
private lemma sum_AC (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, A5 x p * C5 y p =
  ...
  -- Type A: xa*(xb*(xb*(xc*(xc*ya)))) = (xa*ya)*(xb*xb)*(xc*xc)
  -- Type B: xa*(xa*(xb*(xc*(xc*yb)))) = (xa*xa)*(xb*yb)*(xc*xc)
  -- Type C: xa*(xa*(xb*(xb*(xc*yc)))) = (xa*xa)*(xb*xb)*(xc*yc)
  have k1 : ∀ a b c : Fin 8,
...
private lemma sum_CA (x y : EuclideanSpace ℝ (Fin 8)) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, C5 x p * A5 y p =
    15 * @inner ℝ _ _ x y := by
  rw [show @inner ℝ _ _ x y = @inner ℝ _ _ y x from real_inner_comm y x]
  simp_rw [show ∀ p, C5 x p * A5 y p = A5 y p * C5 x p from fun p => mul_comm _ _]
  exact sum_AC y x hy

-- B5 decomposition: first 4 terms (delta involves position a) and last 6 terms
private def B5a (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
    x p.2.2.1 * x p.2.2.2.1 * x p.2.2.2.2 * (if p.1 = p.2.1 then 1 else 0)
  + x p.2.1 * x p.2.2.2.1 * x p.2.2.2.2 * (if p.1 = p.2.2.1 then 1 else 0)
  + x p.2.1 * x p.2.2.1 * x p.2.2.2.2 * (if p.1 = p.2.2.2.1 then 1 else 0)
...
private def B5b (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
    x p.1 * x p.2.2.2.1 * x p.2.2.2.2 * (if p.2.1 = p.2.2.1 then 1 else 0)
  + x p.1 * x p.2.2.1 * x p.2.2.2.2 * (if p.2.1 = p.2.2.2.1 then 1 else 0)
  + x p.1 * x p.2.2.1 * x p.2.2.2.1 * (if p.2.1 = p.2.2.2.2 then 1 else 0)
...
private lemma B5_split (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) :
    B5 x p = B5a x p + B5b x p := by
  simp only [B5, B5a, B5b]; ring

set_option maxHeartbeats 1600000 in
private lemma sum_B5aa (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5a x p * B5a y p =
    44 * (@inner ℝ _ _ x y) ^ 3 := by
  set s := @inner ℝ _ _ x y
  have hs : s = ∑ a : Fin 8, x.ofLp a * y.ofLp a := inner_ofLp x y
  simp only [B5a]
  simp_rw [Fintype.sum_prod_type]
...
set_option maxHeartbeats 3200000 in
private lemma sum_B5ab (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5a x p * B5b y p =
  ...
  have v1 : ∑ a : Fin 8, ∑ b : Fin 8, ∑ c : Fin 8,
    ...
    · exact factor3 _ _ _
    · rw [← hs]; ring
  have v2 : ∑ a : Fin 8, ∑ b : Fin 8, ∑ c : Fin 8,
    ...
    · exact factor3 _ _ _
    · rw [hxn, hyn, ← hs]; ring
  have v3 : ∑ a : Fin 8, ∑ b : Fin 8, ∑ c : Fin 8,
...
private lemma sum_B5bb (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5b x p * B5b y p =
    72 * (@inner ℝ _ _ x y) ^ 3 + 6 * @inner ℝ _ _ x y := by
  set s := @inner ℝ _ _ x y
  have hs : s = ∑ a : Fin 8, x.ofLp a * y.ofLp a := inner_ofLp x y
  have hxn := ofLp_norm_sq x hx
  have hyn := ofLp_norm_sq y hy
  ...
  simp only [Finset.sum_ite_eq, Finset.sum_ite_eq', Finset.mem_univ, ite_true,
    sum_ite_const_zero]
  simp only [mul_assoc, mul_comm, mul_left_comm]
  -- k-lemma: all products have the same AC-normal form after mul_assoc/mul_comm
  -- but we need to map them to factor3-compatible form
  -- The products are: x_a*(x_b*(x_c*(y_a*(y_b*y_c)))) and x_a*(x_b*(x_b*(y_a*(y_c*y_c))))
  -- plus same with different index names
  -- Strategy: map each to (f a)*(g b)*(h c) form, then use v-lemmas
  -- First handle kA form (all paired x_i*y_i):
  have kA : ∀ a b c : Fin 8,
      x.ofLp a * (x.ofLp b * (x.ofLp c * (y.ofLp a * (y.ofLp b * y.ofLp c)))) =
      (x.ofLp a * y.ofLp a) * (x.ofLp b * y.ofLp b) * (x.ofLp c * y.ofLp c) := by
    intros; ring
  -- kB form: x_a paired with y_a, x_b doubled, y_c doubled
  have kB : ∀ a b c : Fin 8,
      x.ofLp a * (x.ofLp b * (x.ofLp b * (y.ofLp a * (y.ofLp c * y.ofLp c)))) =
      (x.ofLp a * y.ofLp a) * (x.ofLp b * x.ofLp b) * (y.ofLp c * y.ofLp c) := by
    intros; ring
  simp_rw [kA, kB]
  -- Factor all sums by pulling constants out, then evaluate
  simp only [Finset.sum_const, Finset.card_univ, Fintype.card_fin, nsmul_eq_mul, Nat.cast_ofNat]
...
private lemma sum_BB (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5 x p * B5 y p =
    140 * (@inner ℝ _ _ x y) ^ 3 + 30 * @inner ℝ _ _ x y := by
  simp_rw [B5_split]
...
private def C5a (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.1 * ((if p.2.1 = p.2.2.1 then 1 else 0) * (if p.2.2.2.1 = p.2.2.2.2 then 1 else 0)
         + (if p.2.1 = p.2.2.2.1 then 1 else 0) * (if p.2.2.1 = p.2.2.2.2 then 1 else 0)
         + (if p.2.1 = p.2.2.2.2 then 1 else 0) * (if p.2.2.1 = p.2.2.2.1 then 1 else 0))

private def C5b (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.2.1 * ((if p.1 = p.2.2.1 then 1 else 0) * (if p.2.2.2.1 = p.2.2.2.2 then 1 else 0)
           + (if p.1 = p.2.2.2.1 then 1 else 0) * (if p.2.2.1 = p.2.2.2.2 then 1 else 0)
           + (if p.1 = p.2.2.2.2 then 1 else 0) * (if p.2.2.1 = p.2.2.2.1 then 1 else 0))

private def C5c (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.2.2.1 * ((if p.1 = p.2.1 then 1 else 0) * (if p.2.2.2.1 = p.2.2.2.2 then 1 else 0)
             + (if p.1 = p.2.2.2.1 then 1 else 0) * (if p.2.1 = p.2.2.2.2 then 1 else 0)
             + (if p.1 = p.2.2.2.2 then 1 else 0) * (if p.2.1 = p.2.2.2.1 then 1 else 0))

private def C5d (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.2.2.2.1 * ((if p.1 = p.2.1 then 1 else 0) * (if p.2.2.1 = p.2.2.2.2 then 1 else 0)
                + (if p.1 = p.2.2.1 then 1 else 0) * (if p.2.1 = p.2.2.2.2 then 1 else 0)
                + (if p.1 = p.2.2.2.2 then 1 else 0) * (if p.2.1 = p.2.2.1 then 1 else 0))

private def C5e (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  x p.2.2.2.2 * ((if p.1 = p.2.1 then 1 else 0) * (if p.2.2.1 = p.2.2.2.1 then 1 else 0)
                + (if p.1 = p.2.2.1 then 1 else 0) * (if p.2.1 = p.2.2.2.1 then 1 else 0)
                + (if p.1 = p.2.2.2.1 then 1 else 0) * (if p.2.1 = p.2.2.1 then 1 else 0))

private lemma C5_split (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) :
    C5 x p = C5a x p + C5b x p + C5c x p + C5d x p + C5e x p := by
  simp only [C5, C5a, C5b, C5c, C5d, C5e]

-- Swap equivalences for 5-tuples: swap position i with position j
private def swapAB : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
  toFun := fun ⟨a, b, c, d, e⟩ => ⟨b, a, c, d, e⟩
  invFun := fun ⟨a, b, c, d, e⟩ => ⟨b, a, c, d, e⟩
  left_inv := fun ⟨_, _, _, _, _⟩ => rfl
  right_inv := fun ⟨_, _, _, _, _⟩ => rfl

private def swapAC : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
private def swapAD : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
private def swapAE : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
private def swapBC : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
private def swapBD : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
private def swapBE : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) where
...
-- Application lemmas for swap Equivs (simp can't reduce Equiv coercions)
private lemma swapAB_apply (a b c d e : Fin 8) :
    swapAB (a, b, c, d, e) = (b, a, c, d, e) := rfl
private lemma swapAC_apply (a b c d e : Fin 8) :
    swapAC (a, b, c, d, e) = (c, b, a, d, e) := rfl
private lemma swapAD_apply (a b c d e : Fin 8) :
    swapAD (a, b, c, d, e) = (d, b, c, a, e) := rfl
private lemma swapAE_apply (a b c d e : Fin 8) :
    swapAE (a, b, c, d, e) = (e, b, c, d, a) := rfl
private lemma swapBC_apply (a b c d e : Fin 8) :
    swapBC (a, b, c, d, e) = (a, c, b, d, e) := rfl
private lemma swapBD_apply (a b c d e : Fin 8) :
    swapBD (a, b, c, d, e) = (a, d, c, b, e) := rfl
private lemma swapBE_apply (a b c d e : Fin 8) :
    swapBE (a, b, c, d, e) = (a, e, c, d, b) := rfl

-- Helper: normalize reversed equality in ite conditions before ring
-- Can't rw on Prop inside ite (motive error), so rewrite the whole ite expression
private lemma ite_eqc (a b : Fin 8) :
...
private lemma B5_inv_swapAE (x : EuclideanSpace ℝ (Fin 8)) (p) :
...
private lemma sum_B5_C5i_eq_C5a (x y : EuclideanSpace ℝ (Fin 8))
    (C5i : EuclideanSpace ℝ (Fin 8) →
      (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) → ℝ)
    (σ : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
...
-- Sub-sum: ∑ B5×C5a = 72s (B5 has 10 terms, C5a has 3 → 30 product terms)
set_option maxHeartbeats 6400000 in
private lemma sum_B5_C5a (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5 x p * C5a y p =
    72 * @inner ℝ _ _ x y := by
  set s := @inner ℝ _ _ x y
  have hs : s = ∑ a : Fin 8, x.ofLp a * y.ofLp a := inner_ofLp x y
  have hxn := ofLp_norm_sq x hx
  simp only [B5, C5a]
...
-- B5 is symmetric in all 5 positions → B5*C5i = B5*C5a via position swap
private lemma sum_BC (x : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1)
    (y : EuclideanSpace ℝ (Fin 8)) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, B5 x p * C5 y p =
    360 * @inner ℝ _ _ x y := by
  simp_rw [C5_split]
...
private lemma sum_CB (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, C5 x p * B5 y p =
    360 * @inner ℝ _ _ x y := by
  rw [show @inner ℝ _ _ x y = @inner ℝ _ _ y x from real_inner_comm y x]
  simp_rw [show ∀ p, C5 x p * B5 y p = B5 y p * C5 x p from fun p => mul_comm _ _]
  exact sum_BC y hy x hx

-- Cross-term: same-unpaired (a,a). Result = 240s
-- C5a(x,p)*C5a(y,p) = x_a*y_a * G(b,c,d,e)^2
-- G^2 has 9 delta products. After summing: 3*64s + 6*8s = 240s
set_option maxHeartbeats 800000 in
private lemma sum_C5aa (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, C5a x p * C5a y p =
    240 * @inner ℝ _ _ x y := by
  set s := @inner ℝ _ _ x y
  have hs : s = ∑ a : Fin 8, x.ofLp a * y.ofLp a := inner_ofLp x y
  simp only [C5a]
  simp_rw [Fintype.sum_prod_type]
...
-- Cross-term: diff-unpaired (a,b). Result = 30s
set_option maxHeartbeats 800000 in
private lemma sum_C5ab (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, C5a x p * C5b y p =
    30 * @inner ℝ _ _ x y := by
  set s := @inner ℝ _ _ x y
  have hs : s = ∑ a : Fin 8, x.ofLp a * y.ofLp a := inner_ofLp x y
  simp only [C5a, C5b]
  simp_rw [Fintype.sum_prod_type]
...
private lemma C5c_eq_C5b_swap (z : EuclideanSpace ℝ (Fin 8)) (p) :
    C5c z p = C5b z (swapBC p) := by
  obtain ⟨a, b, c, d, e⟩ := p; simp only [C5c, C5b, swapBC_apply]

private lemma C5d_eq_C5b_swap (z : EuclideanSpace ℝ (Fin 8)) (p) :
    C5d z p = C5b z (swapBD p) := by
  obtain ⟨a, b, c, d, e⟩ := p; simp only [C5d, C5b, swapBD_apply]
  rw [ite_eqc c b]; ring

private lemma C5e_eq_C5b_swap (z : EuclideanSpace ℝ (Fin 8)) (p) :
...
private lemma sum_C5ii_eq_aa (z w : EuclideanSpace ℝ (Fin 8))
    (C5i : EuclideanSpace ℝ (Fin 8) →
      (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) → ℝ)
    (σ : (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) ≃
      (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8))
...
-- Off-diagonal sum identity: ∑ C5i×C5j = ∑ C5a×C5b via composed swaps
-- Strategy: for off-diag (i,j), find σ s.t. C5i(z,p)=C5a(z,σp) and C5j(z,p)=C5b(z,σp)
-- Key insight: swapAB maps C5b→C5a and preserves C5a→C5b? No.
-- Instead: for each off-diag pair, show the product sum equals sum_C5ab directly.
-- Approach: convert via two independent swaps that commute.

-- Helper: any cross-term with C5a on one side and C5j on other = sum_C5ab
-- because swapping j↔b maps C5j→C5b while preserving C5a
private lemma sum_C5a_C5c (x y : EuclideanSpace ℝ (Fin 8)) :
...
private lemma sum_C5a_C5d (x y : EuclideanSpace ℝ (Fin 8)) :
...
private lemma sum_C5a_C5e (x y : EuclideanSpace ℝ (Fin 8)) :
...
-- Reverse helper: ∑ C5j×C5i = 30s from ∑ C5i×C5j = 30s
private lemma sum_C5_rev {C5i C5j : EuclideanSpace ℝ (Fin 8) →
    (Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) → ℝ}
    (h : ∀ x y : EuclideanSpace ℝ (Fin 8),
      ∑ p, C5i x p * C5j y p = 30 * @inner ℝ _ _ x y)
    (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5j x p * C5i y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5b_C5c (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5b x p * C5c y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5b_C5d (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5b x p * C5d y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5b_C5e (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5b x p * C5e y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5c_C5d (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5c x p * C5d y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5c_C5e (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5c x p * C5e y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_C5d_C5e (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p, C5d x p * C5e y p = 30 * @inner ℝ _ _ x y := by
...
private lemma sum_CC (x y : EuclideanSpace ℝ (Fin 8)) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, C5 x p * C5 y p =
    1800 * @inner ℝ _ _ x y := by
  -- Expand C5 into 5 groups
  simp_rw [C5_split]
  simp only [add_mul, mul_add, Finset.sum_add_distrib]
  -- All 5 diagonal terms = sum_C5aa = 240s
...
private lemma phi5_kernel (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, phi5 x p * phi5 y p =
    (3 : ℝ) / 8 * P8 5 (@inner ℝ _ _ x y) := by
  suffices h : ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, phi5 x p * phi5 y p =
      (@inner ℝ _ _ x y) ^ 5 - 5 / 7 * (@inner ℝ _ _ x y) ^ 3
      + 5 / 56 * @inner ℝ _ _ x y by
    rw [h, P8_5]; ring
  set s := @inner ℝ _ _ x y
...
def phi5Feature (x : EuclideanSpace ℝ (Fin 8))
    (p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8) : ℝ :=
  phi5 x p

lemma phi5Feature_kernel (x y : EuclideanSpace ℝ (Fin 8)) (hx : ‖x‖ = 1) (hy : ‖y‖ = 1) :
    ∑ p : Fin 8 × Fin 8 × Fin 8 × Fin 8 × Fin 8, phi5Feature x p * phi5Feature y p =
    (3 : ℝ) / 8 * P8 5 (@inner ℝ _ _ x y) := by
  simp only [phi5Feature]
