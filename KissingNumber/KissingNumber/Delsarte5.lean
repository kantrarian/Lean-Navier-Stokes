...
/-!
# Delsarte Linear Programming Bound for Dimension 5

This file establishes the Delsarte LP bound framework for d=5:
...
2. The Delsarte bound theorem: given a certificate polynomial with PSD hypothesis,
   any such configuration has at most B points (where B = f(1)/c₀).
...
/-! ## Bridge Lemma: Kissing Configuration → Unit Sphere (dimension 5) -/

/-- From a kissing configuration (norm 2, dist ≥ 2) in dimension 5, we extract unit vectors
    with inner products ≤ 1/2 by scaling by 1/2. -/
theorem kissing_to_unit_sphere_5 {N : ℕ} {X : Fin N → EuclideanSpace ℝ (Fin 5)}
    (hX : IsKissingConfiguration 5 N X) :
    ∃ u : Fin N → EuclideanSpace ℝ (Fin 5),
      (∀ i, ‖u i‖ = 1) ∧
...
/-! ## Inner product bounds for unit vectors in dimension 5 -/

/-- For unit vectors in dimension 5, inner product is at least -1 (from Cauchy-Schwarz). -/
lemma neg_one_le_inner_of_unit_5 {u v : EuclideanSpace ℝ (Fin 5)}
    (hu : ‖u‖ = 1) (hv : ‖v‖ = 1) :
...
/-- The self inner product of a unit vector in dimension 5 is 1. -/
lemma inner_self_eq_one_of_unit_5 {u : EuclideanSpace ℝ (Fin 5)}
    (hu : ‖u‖ = 1) :
    @inner ℝ _ _ u u = 1 := by
  rw [real_inner_self_eq_norm_sq, hu, one_pow]

/-! ## Delsarte Bound Theorem for Dimension 5 -/

/--
**Delsarte LP bound for dimension 5.**
...
any unit-sphere configuration with pairwise inner products ≤ 1/2 has ≤ B points,
where B is determined by f(1)/c₀.
-/
theorem delsarte_bound_5
    (f : ℝ → ℝ) (c : Fin 6 → ℝ) (B : ℕ)
    (hexp : ∀ t, f t = ∑ k : Fin 6, c k * P5 k t)
    (hpos : ∀ k : Fin 6, k ≠ 0 → 0 ≤ c k)
    (hc0 : 0 < c 0)
    (hneg : ∀ t, t ∈ Set.Icc (-1 : ℝ) (1 / 2) → f t ≤ 0)
    (hf1 : f 1 = c 0 * B)
    (N : ℕ) (u : Fin N → EuclideanSpace ℝ (Fin 5))
    (hunit : ∀ i, ‖u i‖ = 1)
    (hinner : ∀ i j, i ≠ j → @inner ℝ _ _ (u i) (u j) ≤ (1 : ℝ) / 2)
    (hpsd : ∀ k : Fin 6, k ≠ 0 →
        0 ≤ ∑ i : Fin N, ∑ j : Fin N, P5 k (@inner ℝ _ _ (u i) (u j))) :
    N ≤ B := by
  -- Handle N = 0 trivially
  by_cases hN : N = 0
  · omega
  have hN_pos : 0 < N := Nat.pos_of_ne_zero hN
  -- Key quantity: S_k := ∑ᵢ ∑ⱼ P5 k ⟪uᵢ, uⱼ⟫
  set S := fun k : Fin 6 => ∑ i : Fin N, ∑ j : Fin N, P5 k (@inner ℝ _ _ (u i) (u j)) with hS_def

  -- Step 1: S 0 = N² (since P5 0 = 1)
  have hS0 : S 0 = (N : ℝ) ^ 2 := by
    simp only [hS_def, P5_0]
    simp only [Finset.sum_const, Finset.card_univ, Fintype.card_fin]
    ring

  -- Step 2: ∑ₖ cₖ · Sₖ = ∑ᵢ ∑ⱼ f(⟪uᵢ,uⱼ⟫)
  have h_total : ∑ k : Fin 6, c k * S k =
  ...
  have h_nonneg_terms : ∀ k : Fin 6, k ≠ 0 → 0 ≤ c k * S k :=
    fun k hk => mul_nonneg (hpos k hk) (hpsd k hk)
  have h_lower : c 0 * (N : ℝ) ^ 2 ≤ ∑ k : Fin 6, c k * S k := by
    rw [← hS0]
    have h_split := Finset.add_sum_erase Finset.univ (fun k => c k * S k) (Finset.mem_univ (0 : Fin 6))
    rw [← h_split]
    linarith [Finset.sum_nonneg (fun k (hk : k ∈ Finset.univ.erase (0 : Fin 6)) =>
  ...
  have h_diag : ∀ i : Fin N, f (@inner ℝ _ _ (u i) (u i)) = f 1 := by
    intro i; congr 1; exact inner_self_eq_one_of_unit_5 (hunit i)
          ...
          calc ∑ j : Fin N, f (@inner ℝ _ _ (u i) (u j))
            ...
            _ ≤ f 1 + 0 := by
                ...
                · apply Finset.sum_nonpos
                  intro j hj
                  apply hneg
                  exact ⟨neg_one_le_inner_of_unit_5 (hunit i) (hunit j),
                         hinner i j (Finset.ne_of_mem_erase hj).symm⟩
            _ = f 1 := add_zero _
      _ = (N : ℝ) * f 1 := by
          simp [Finset.sum_const, Finset.card_univ, Fintype.card_fin]

  -- Step 5: Combine: c₀ · N² ≤ N · f(1) = N · c₀ · B
  have h_combined : c 0 * (N : ℝ) ^ 2 ≤ (N : ℝ) * (c 0 * B) := by
    calc c 0 * (N : ℝ) ^ 2 ≤ ∑ k : Fin 6, c k * S k := h_lower
      _ = ∑ i : Fin N, ∑ j : Fin N, f (@inner ℝ _ _ (u i) (u j)) := h_total
      _ ≤ (N : ℝ) * f 1 := h_upper
      _ = (N : ℝ) * (c 0 * B) := by rw [hf1]

  -- Step 6: Divide by c₀ · N (both positive): N ≤ B
  have hN_pos_real : (0 : ℝ) < N := Nat.cast_pos.mpr hN_pos
  have hcN : 0 < c 0 * (N : ℝ) := mul_pos hc0 hN_pos_real
  have h_le_real : (N : ℝ) ≤ B := by nlinarith
