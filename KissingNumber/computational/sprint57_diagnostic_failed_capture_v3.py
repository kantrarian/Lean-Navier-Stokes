...
def attempt_insertion_certified(
    ...
    best_min_gap_raw = -np.inf
    best_config_proj = None
    best_min_gap_proj = -np.inf
    ...
    for r in r_vals:
        ...
        # Current configuration (intruder stays off-sphere during approach)
        all_points_raw = np.vstack([shell, intruder])

        # Gap check at off-sphere intruder position (optimistic)
        _, _, min_gap_raw, _ = check_configuration(all_points_raw, R, eps_r=0.1, eps_g=0.1)

        # Gap check after projecting intruder to the kissing sphere (what certification "sees")
        intruder_proj = project_to_sphere(intruder, radius)
        all_points_proj = np.vstack([shell, intruder_proj])
        _, _, min_gap_proj, _ = check_configuration(all_points_proj, R, eps_r=0.1, eps_g=0.1)

        trajectory["r_values"].append(r)
        trajectory["min_gaps_raw"].append(min_gap_raw)
        trajectory["min_gaps_projected"].append(min_gap_proj)

        # Track Xi0 on the projected configuration to match certification geometry
        if track_xi0 and prev_all_points is not None:
            xi0, delta_min = compute_xi0(all_points_proj, prev_all_points, R=R, omega=omega)
        ...
        prev_all_points = all_points_proj.copy()

        # Track best configurations encountered (raw vs projected)
        if min_gap_raw > best_min_gap_raw:
            best_min_gap_raw = min_gap_raw
            best_config_raw = all_points_raw.copy()

        if min_gap_proj > best_min_gap_proj:
            best_min_gap_proj = min_gap_proj
            best_config_proj = all_points_proj.copy()

        # Stop if deep overlap after projection (unsalvageable for certification)
        if min_gap_proj < -0.4:
            break

    # Trajectory summary (even for failed approaches)
    if trajectory["min_gaps_raw"]:
        traj_min_gap_max_raw = float(np.max(trajectory["min_gaps_raw"]))
        traj_min_gap_min_raw = float(np.min(trajectory["min_gaps_raw"]))
    else:
        traj_min_gap_max_raw = None
        traj_min_gap_min_raw = None

    if trajectory["min_gaps_projected"]:
        traj_min_gap_max_projected = float(np.max(trajectory["min_gaps_projected"]))
        traj_min_gap_min_projected = float(np.min(trajectory["min_gaps_projected"]))
    else:
        traj_min_gap_max_projected = None
        traj_min_gap_min_projected = None

    # Approach result (projected is the relevant channel for certification)
    approach_best_gap_raw = best_min_gap_raw
    approach_best_gap_projected = best_min_gap_proj
    approach_success = (best_min_gap_proj > -0.2)

    if best_config_proj is None:
        return False, None, {
            "approach_success": False,
            "approach_best_gap_raw": approach_best_gap_raw,
            "approach_best_gap_projected": approach_best_gap_projected,
            "traj_min_gap_max_raw": traj_min_gap_max_raw,
            "traj_min_gap_min_raw": traj_min_gap_min_raw,
            "traj_min_gap_max_projected": traj_min_gap_max_projected,
            "traj_min_gap_min_projected": traj_min_gap_min_projected,
            "best_approach_config": None,
            "best_approach_config_raw": best_config_raw,
            "reason": "no_config",
            "trajectory": trajectory
        }
    # CERTIFICATION PHASE (starts from best projected approach config)
    certified, final_config, cert_diag = certify_configuration(
        best_config_proj, R=R,
    ...
    if certified and track_xi0:
        # Compute Xi0 at certified configuration vs pre-certification (projected approach)
        final_xi0, final_delta_min = compute_xi0(final_config, best_config_proj, R=R, omega=omega)
    return certified, final_config, {
        ...
        "traj_min_gap_max_projected": traj_min_gap_max_projected,
        "traj_min_gap_min_projected": traj_min_gap_min_projected,
        "best_approach_config": best_config_proj,
        "best_approach_config_raw": best_config_raw,
        "certified": certified,
    ...
    for s in range(n_seeds):
        ...
        while current_N < N_target:
            ...
            for u in seam_dirs:
                ...
                # Always track the best approach seen (including failed approaches)
                cand_gap_proj = diag.get("traj_min_gap_max_projected", diag.get("approach_best_gap_projected", -np.inf))
                cand_gap_raw = diag.get("traj_min_gap_max_raw", diag.get("approach_best_gap_raw", -np.inf))
                cand_config = final_config if (certified and final_config is not None) else diag.get("best_approach_config")
                if cand_config is not None and cand_gap_proj > best_approach_gap:
                    best_approach_gap = cand_gap_proj
                    best_approach_config = cand_config.copy()
                    best_approach_diag = diag
                    # Store raw gap too for later reporting
                    best_approach_diag["_cand_gap_raw"] = cand_gap_raw
                    ...
                    if first_failure_reason is None:
                        ...
                        if reason is None:
                            ...
                            if cert_reason is not None:
                                ...
                                mins = (diag.get("trajectory") or {}).get("min_gaps_projected") or []
            ...
            insertion_info = {
                ...
                # Gap summaries: projected is what certification geometry sees
                "best_approach_gap_projected": best_approach_gap if best_approach_gap > -np.inf else None,
                "best_approach_gap_raw": (best_approach_diag.get("_cand_gap_raw") if best_approach_diag is not None else None),
                "best_traj_min_gap_min_projected": (best_approach_diag.get("traj_min_gap_min_projected") if best_approach_diag is not None else None),
                "best_traj_min_gap_min_raw": (best_approach_diag.get("traj_min_gap_min_raw") if best_approach_diag is not None else None),
