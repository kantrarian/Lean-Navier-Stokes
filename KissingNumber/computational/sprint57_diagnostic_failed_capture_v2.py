...
def attempt_insertion_certified(
    ...
    for r in r_vals:
        ...
        all_points = np.vstack([shell, intruder])
    ...
    if trajectory["min_gaps"]:
        traj_min_gap_max = float(np.max(trajectory["min_gaps"]))
        traj_min_gap_min = float(np.min(trajectory["min_gaps"]))
    else:
        traj_min_gap_max = None
        traj_min_gap_min = None
    ...
    if best_config is None:
        return False, None, {
            ...
            "traj_min_gap_max": traj_min_gap_max,
            "traj_min_gap_min": traj_min_gap_min,
    ...
    return certified, final_config, {
        ...
        "traj_min_gap_max": traj_min_gap_max,
        "traj_min_gap_min": traj_min_gap_min,
        "best_approach_config": best_config,
        "certified": certified,
    ...
    for s in range(n_seeds):
        ...
        while current_N < N_target:
            ...
            for u in seam_dirs:
                ...
                cand_gap = diag.get("traj_min_gap_max", diag.get("approach_best_gap", -np.inf))
                cand_config = final_config if (certified and final_config is not None) else diag.get("best_approach_config")
                if cand_config is not None and cand_gap > best_approach_gap:
                    best_approach_gap = cand_gap
            ...
            insertion_info = {
                ...
                "best_traj_min_gap_min": (best_approach_diag.get("traj_min_gap_min") if best_approach_diag is not None else None),
