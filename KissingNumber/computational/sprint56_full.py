"""
Sprint 5.6: Certification + Falsification (FULL VERSION)
=========================================================
Production-quality certification for 5D kissing number investigation.

NO SHORTCUTS - designed for local execution with full compute.

Key features:
1. Strict certification: eps_r = eps_g = 1e-5 (tight)
2. High search intensity: K_seams=50, n_seeds=50
3. N=45 falsification (MUST fail if constraints correct)
4. Multiple initialization strategies
5. Xi0 tracking on certified configurations
6. Comprehensive diagnostics and logging
7. Intermediate result saving

Known bounds: 40 ≤ τ₅ ≤ 44
- D₅ root system gives 40 kissing spheres
    ...
    for s in range(n_seeds):
        ...
        while current_N < N_target:
            ...
            for u in seam_dirs:
                ...
                if diag["approach_success"]:
                    approach_gap = diag["approach_best_gap"]
                    if approach_gap > best_approach_gap:
                        best_approach_gap = approach_gap
            ...
            insertion_info = {
                ...
                "best_approach_gap": best_approach_gap
            }
            seed_result["insertion_details"].append(insertion_info)
            
            # Prefer certified, then try to certify best approach
            if best_certified_config is not None:
                shell = best_certified_config
                current_N += 1
            elif best_approach_config is not None:
                # One more certification attempt with more iterations
                certified_retry, final_config, _ = certify_configuration(
    ...
    for N in config["N_values"]:
        ...
        with open(intermediate_path, 'w') as f:
            json.dump({
                "N": N,
                "summary": summary,
                "results": [{k: v for k, v in r.items() if k != "insertion_details"} 
                           for r in results]
...
def analyze_certification_results(all_summaries, output_dir="./sprint56_results"):
    ...
    # Save plot
    plot_path = output_dir / "sprint56_certification.png"
...
if __name__ == "__main__":
    # Run full suite
    all_results, all_summaries = run_full_certification_suite(
        config=CONFIG,
        verbose=True
    )
    ...
    summary_path = output_dir / "sprint56_final_summary.json"
    with open(summary_path, 'w') as f:
        json.dump(final_summary, f, indent=2, default=str)
    print(f"\nFinal summary saved to: {summary_path}")
    
    print("\n" + "=" * 80)
    print("SPRINT 5.6 COMPLETE")
    print("=" * 80)
