...
### Gram Matrix Optimization (Sprint 5.11-C)
By optimizing the Gram matrix $G \succeq 0$ directly (minimizing maximum off-diagonal entry), we avoided coordinate projection artifacts.
- **Max Cosine**: 0.509
- **Min Angle**: 59.39° (Target: 60°)
- **Gap**: **-0.018** (vs -0.036 for coordinate search)

**Finding**: The "coordinate-free" relaxation gets closer to feasibility but still hits a barrier at -0.018 gap (~0.9% diameter overlap). This refines our lower bound estimates but maintains the negative result.
