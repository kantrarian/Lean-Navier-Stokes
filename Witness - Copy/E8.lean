...
-- Inner product of Type2 roots is bounded by 1
-- Each coordinate gives ±1/4, summing to ≤1 when at least 2 coordinates differ
-- Type2 roots have even parity, so XOR has even Hamming weight (≥2 if nonzero)
-- This is proved by a parity argument: the 8th bit enforces even popcount
-- The proof is complex due to Mathlib API issues with Finset/set comprehension unification
-- Key insight: inner product = (8 - 2d)/4 = 2 - d/2 where d ≥ 2 for distinct vectors
axiom inner_root2_bound : ∀ (x y : Type2Index), x ≠ y →
    ⟪root2 x, root2 y⟫ ≤ (1 : ℝ)
