...
Beyond the kissing number problem, the spherical code optimization framework developed here has potential applications in machine learning, particularly for approximate nearest neighbor (ANN) search and vector quantization. The gap metric $g_{\min}$ directly measures codebook separation quality, and the control-first methodology transfers to guarantee-first codebook design.

Two concrete extensions merit investigation:
\begin{enumerate}
\item \textbf{Data-driven codebook optimization:} Adapt the annealed optimizer to jointly minimize quantization error on a data distribution while maintaining separation guarantees via a penalty term $\lambda \max(0, -g)$. This would provide codebooks with certified minimum separation, potentially improving IVF indexing and product quantization systems.
\item \textbf{Initialization benchmark:} Compare spherical code initialization against random initialization for k-means clustering on normalized embeddings (e.g., SIFT1M, Deep1B). Metrics would include k-means convergence rate, final MSE, and recall@k for nearest neighbor search.
\end{enumerate}
The dimensional analysis in Table~\ref{tab:calibration} may also illuminate why $k$-NN search becomes harder in certain dimensions: the gap at $\tau+1$ measures geometric ``room'' for additional points, with $d=5$ showing the tightest margin (3.6\%) among tested dimensions.
